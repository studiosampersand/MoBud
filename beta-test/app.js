const VERSION='0.000.008-hotfix6';
...
function uidDevice(){return `device_${Date.now().toString(36)}${Math.random().toString(36).slice(2,10)}`}
...
function logHtml(x){const v=vehicleById(x.vehicleId),type=v?.type||(x.kind==='expense'?'other':'other'),attachment=x.attachment?'<span title="Attachment"> · attachment</span>':'';const title=x.kind==='trip'?friendlyWhen(x.date)+(x.createdAt?' '+new Date(x.createdAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):''):x.category;const subtitle=x.kind==='trip'?(v?.name||'No specific vehicle'):(v?.name||x.supplier||'General cost');const value=x.kind==='trip'?unitDistance(x.km):money(x.amount);return `<div class="log-row"><div class="log-icon">${iconType(type)}</div><div class="log-main"><strong>${title}</strong><small>${subtitle}${attachment}</small></div><div class="log-side"><strong>${value}</strong><div class="log-actions"><button class="ghost" data-edit-${x.kind}="${x.id}">Edit</button><button class="danger" data-delete-${x.kind}="${x.id}">Delete</button></div></div></div>`}
...
function renderGarage(){const typeFilter=document.getElementById('garageTypeFilter')?.value||'all',powerFilter=document.getElementById('garagePowertrainFilter')?.value||'all';const list=state.vehicles.filter(v=>(typeFilter==='all'||v.type===typeFilter)&&(powerFilter==='all'||(v.powertrain||'not_applicable')===powerFilter));if(!list.length){garageList.innerHTML='<div class="card empty-state">No matching vehicles.</div>';return}garageList.innerHTML=list.map(v=>{const trips=state.trips.filter(t=>t.vehicleId===v.id),logs=state.expenses.filter(e=>e.vehicleId===v.id);const title=[v.brand,v.model].filter(Boolean).join(' ')||v.name;return `<div class="card garage-card"><div class="section-head"><div class="vehicle-title"><span class="vehicle-icon">${iconType(v.type)}</span><div><h3>${title}</h3><strong>${v.name}</strong><p class="muted">${String(v.type).replaceAll('_',' ')} · ${String(v.powertrain||'not_applicable').replaceAll('_',' ')}</p>${v.isDefault?'<span class="default-badge">DEFAULT</span>':''}</div></div><div class="log-actions"><button class="ghost" data-edit-vehicle="${v.id}">Edit</button><button class="ghost" data-archive-vehicle="${v.id}">${v.active===false?'Restore':'Archive'}</button></div></div><div class="vehicle-meta"><div><small>Distance logged</small>${unitDistance(trips.reduce((s,t)=>s+Number(t.km||0),0))}</div><div><small>Tickets & logs</small>${logs.length}</div><div><small>Total cost</small>${money(logs.reduce((s,e)=>s+Number(e.amount||0),0))}</div><div><small>Status</small>${v.active===false?'Archived':'Active'}</div></div>${['bicycle','ebike','speed_pedelec','car','van','motorcycle','scooter'].includes(v.type)?maintenanceTimeline(v):''}${v.leasePartner?`<p>Lease partner: ${link(v.leasePartner,v.leaseUrl)}</p>`:''}${v.maintenanceProvider?`<p>Maintenance: ${link(v.maintenanceProvider,v.maintenanceUrl)}</p>`:''}${v.insurance?`<p>Insurance: ${v.insurance}</p>`:''}<button class="secondary small" data-vehicle-logs="${v.id}">See all logs for this vehicle</button></div>`}).join('');document.querySelectorAll('[data-edit-vehicle]').forEach(b=>b.onclick=()=>openVehicle(null,b.dataset.editVehicle));document.querySelectorAll('[data-archive-vehicle]').forEach(b=>b.onclick=()=>archiveVehicle(b.dataset.archiveVehicle));document.querySelectorAll('[data-vehicle-logs]').forEach(b=>b.onclick=()=>showVehicleLogs(b.dataset.vehicleLogs))}
...
async function renderFaq(){
  ...
  try{
    ...
    faqList.innerHTML=topItems.map((item,i)=>{
      ...
      return `<div class="faq-item"><button data-faq="${i}">${escapeHtml(question)}</button><div class="faq-answer hidden" id="faq_${i}">${escapeHtml(answer)} ${tutorial?`<button class="ghost small" data-tutorial-start="${escapeHtml(tutorial)}">Show tutorial</button>`:''}</div></div>`;
    }).join('')+`<a class="secondary link-btn" href="/faq/">View all FAQ</a>`;
    document.querySelectorAll('[data-faq]').forEach(b=>b.onclick=()=>document.getElementById('faq_'+b.dataset.faq).classList.toggle('hidden'));
...
async function downloadDriveJson(fileId){const r=await driveFetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?alt=media`);if(!r.ok)throw new Error(`Drive download failed (${r.status})`);return r.json()}
...
function mergeState(localData,remoteData){const local=normalized(localData),remote=normalized(remoteData),merged=normalized(local);for(const key of ['vehicles','trips','expenses']){const result=mergeCollection(local[key],remote[key],local.tombstones[key],remote.tombstones[key]);merged[key]=result.rows;merged.tombstones[key]=result.deleted}const localSettingsTime=Date.parse(local.syncMeta?.settingsUpdatedAt||local.syncMeta?.modifiedAt||0)||0,remoteSettingsTime=Date.parse(remote.syncMeta?.settingsUpdatedAt||remote.syncMeta?.modifiedAt||0)||0;if(remoteSettingsTime>localSettingsTime){merged.settings={...merged.settings,...remote.settings};merged.addresses=remote.addresses||merged.addresses;merged.selectedDirection=remote.selectedDirection||merged.selectedDirection}merged.routeCache={...(remote.routeCache||{}),...(local.routeCache||{})};merged.syncMeta={...(merged.syncMeta||{}),modifiedAt:new Date(Math.max(Date.parse(local.syncMeta?.modifiedAt||0)||0,Date.parse(remote.syncMeta?.modifiedAt||0)||0)).toISOString(),settingsUpdatedAt:new Date(Math.max(localSettingsTime,remoteSettingsTime)).toISOString(),updatedByDevice:DEVICE_ID,changeToken:local.syncMeta?.changeToken||remote.syncMeta?.changeToken||''};merged.settings.driveConnected=true;merged.settings.storageMode='drive';return merged}
