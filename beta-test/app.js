...
function logHtml(x){const v=vehicleById(x.vehicleId),type=v?.type||(x.kind==='expense'?'other':'other'),attachment=x.attachment?'<span title="Attachment"> · attachment</span>':'';const title=x.kind==='trip'?friendlyWhen(x.date)+(x.createdAt?' '+new Date(x.createdAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):''):cleanText(x.category,200);const subtitle=x.kind==='trip'?(v?.name||'No specific vehicle'):(v?.name||x.supplier||'General cost');const value=x.kind==='trip'?unitDistance(x.km):money(x.amount);const kind=x.kind==='expense'?'expense':'trip';return `<div class="log-row"><div class="log-icon">${iconType(type)}</div><div class="log-main"><strong>${escapeHtml(title)}</strong><small>${escapeHtml(subtitle)}${attachment}</small></div><div class="log-side"><strong>${escapeHtml(value)}</strong><div class="log-actions"><button class="ghost" data-edit-${kind}="${safeAttr(x.id)}">Edit</button><button class="danger" data-delete-${kind}="${safeAttr(x.id)}">Delete</button></div></div></div>`}
...
function fillVehicleSelects(){const opts=['<option value="">No specific vehicle</option>',...state.vehicles.filter(v=>v.active!==false).map(v=>`<option value="${safeAttr(v.id)}">${escapeHtml(v.name)}${v.isDefault?' · default':''}</option>`)].join('');tripVehicle.innerHTML=opts;expenseVehicle.innerHTML=opts}
...
function renderGarage(){const typeFilter=document.getElementById('garageTypeFilter')?.value||'all',powerFilter=document.getElementById('garagePowertrainFilter')?.value||'all';const list=state.vehicles.filter(v=>(typeFilter==='all'||v.type===typeFilter)&&(powerFilter==='all'||(v.powertrain||'not_applicable')===powerFilter));if(!list.length){garageList.innerHTML='<div class="card empty-state">No matching vehicles.</div>';return}garageList.innerHTML=list.map(v=>{const trips=state.trips.filter(t=>t.vehicleId===v.id),logs=state.expenses.filter(e=>e.vehicleId===v.id);const title=[v.brand,v.model].filter(Boolean).join(' ')||v.name;return `<div class="card garage-card"><div class="section-head"><div class="vehicle-title"><span class="vehicle-icon">${iconType(v.type)}</span><div><h3>${escapeHtml(title)}</h3><strong>${escapeHtml(v.name)}</strong><p class="muted">${String(v.type).replaceAll('_',' ')} · ${String(v.powertrain||'not_applicable').replaceAll('_',' ')}</p>${v.isDefault?'<span class="default-badge">DEFAULT</span>':''}</div></div><div class="log-actions"><button class="ghost" data-edit-vehicle="${safeAttr(v.id)}">Edit</button><button class="ghost" data-archive-vehicle="${safeAttr(v.id)}">${v.active===false?'Restore':'Archive'}</button></div></div><div class="vehicle-meta"><div><small>Distance logged</small>${unitDistance(trips.reduce((s,t)=>s+Number(t.km||0),0))}</div><div><small>Tickets & logs</small>${logs.length}</div><div><small>Total cost</small>${money(logs.reduce((s,e)=>s+Number(e.amount||0),0))}</div><div><small>Status</small>${v.active===false?'Archived':'Active'}</div></div>${['bicycle','ebike','speed_pedelec','car','van','motorcycle','scooter'].includes(v.type)?maintenanceTimeline(v):''}${v.leasePartner?`<p>Lease partner: ${link(v.leasePartner,v.leaseUrl)}</p>`:''}${v.maintenanceProvider?`<p>Maintenance: ${link(v.maintenanceProvider,v.maintenanceUrl)}</p>`:''}${v.insurance?`<p>Insurance: ${escapeHtml(v.insurance)}</p>`:''}<button class="secondary small" data-vehicle-logs="${safeAttr(v.id)}">See all logs for this vehicle</button></div>`}).join('');document.querySelectorAll('[data-edit-vehicle]').forEach(b=>b.onclick=()=>openVehicle(null,b.dataset.editVehicle));document.querySelectorAll('[data-archive-vehicle]').forEach(b=>b.onclick=()=>archiveVehicle(b.dataset.archiveVehicle));document.querySelectorAll('[data-vehicle-logs]').forEach(b=>b.onclick=()=>showVehicleLogs(b.dataset.vehicleLogs))}
