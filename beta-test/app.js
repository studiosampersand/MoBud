...
const MAX_BACKUP_BYTES=100_000_000,MAX_ROWS=50_000,MAX_TEXT=5000,ALLOWED_ATTACHMENT_TYPES=new Set(['image/jpeg','image/png','image/webp','application/pdf']);
...
function safeAttachment(a){
  ...
  const type=cleanText(a.type,100);
  if(!ALLOWED_ATTACHMENT_TYPES.has(type))return null;
  const data=typeof a.data==='string'?a.data:'';
  const id=cleanText(a.id||a.attachmentId||'',120)||uid('att');
  const name=cleanText(a.name||a.filename||'attachment',180).replace(/[\/\\]/g,'_');
  const date=cleanText(a.date||a.createdAt||new Date().toISOString(),50);
  const size=Number(a.size||a.bytes||attachmentBytes({data})||0);
  if(data){
    if(!data.startsWith(`data:${type};base64,`)||data.length>60_000_000)return null;
    return {id,name,type,data,date,size,storage:a.storage||'inline'};
  }
  if(id)return {id,name,type,date,size,storage:a.storage||'indexeddb'};
  return null;
}
const ATTACHMENT_DB_NAME=KEY.includes('beta')?'mobud-beta-attachments-v1':'mobud-production-attachments-v1';
const ATTACHMENT_STORE='files';
const MAX_LOCAL_BACKUPS=5;
let attachmentDbPromise=null,attachmentWriteQueue=Promise.resolve(),storageInfoCache={attachmentBytes:0,backupCount:0};
function openAttachmentDb(){
  ...
  if(!attachmentDbPromise){
    attachmentDbPromise=new Promise((resolve,reject)=>{
      ...
      req.onupgradeneeded=()=>{const db=req.result;if(!db.objectStoreNames.contains(ATTACHMENT_STORE))db.createObjectStore(ATTACHMENT_STORE,{keyPath:'id'})};
...
async function idbGet(id){
  ...
  try{
    ...
    return await new Promise((resolve,reject)=>{
      const req=db.transaction(ATTACHMENT_STORE,'readonly').objectStore(ATTACHMENT_STORE).get(id);
...
async function idbDelete(id){
  ...
  }catch(e){console.warn('Attachment delete failed',e)}
}
async function idbAll(prefix=''){
  try{
    const db=await openAttachmentDb();
    return await new Promise((resolve,reject)=>{
      const out=[];
      const req=db.transaction(ATTACHMENT_STORE,'readonly').objectStore(ATTACHMENT_STORE).openCursor();
      req.onsuccess=()=>{const cursor=req.result;if(cursor){if(!prefix||String(cursor.key).startsWith(prefix))out.push(cursor.value);cursor.continue()}else resolve(out)};
...
function attachmentBytes(a){
  if(!a)return 0;
  if(Number(a.size)>0)return Number(a.size);
  const b64=String(a.data||'').split(',')[1]||'';
  return Math.round(b64.length*0.75);
}
function attachmentMeta(safe){return {id:safe.id,name:safe.name,type:safe.type,date:safe.date,size:attachmentBytes(safe),storage:'indexeddb'}}
function stripAttachmentForState(a){
  const safe=safeAttachment(a);
  if(!safe)return null;
  // Never strip inline data during a normal synchronous save. The explicit async
  // migration/import flow stores the bytes first, then replaces this with metadata.
  if(safe.data)return safe;
  return attachmentMeta(safe);
}
function queueAttachmentPersist(a){
  const safe=safeAttachment(a);
  if(!safe?.data)return Promise.resolve(stripAttachmentForState(safe));
  const rec={id:safe.id,name:safe.name,type:safe.type,date:safe.date,size:attachmentBytes(safe),data:safe.data,updatedAt:new Date().toISOString()};
  attachmentWriteQueue=attachmentWriteQueue.then(()=>idbPut(rec)).catch(err=>{console.warn('Attachment IndexedDB write failed',err);toast(t('Attachment storage failed. Metadata was kept.'))});
  return attachmentWriteQueue.then(()=>attachmentMeta(safe));
}
function lightweightState(source=state){
  const clone={...source,settings:{...(source.settings||{})},syncMeta:{...(source.syncMeta||{})},tombstones:{...(source.tombstones||{})},addresses:{...(source.addresses||{})},routeCache:{...(source.routeCache||{})}};
  clone.vehicles=Array.isArray(source.vehicles)?source.vehicles.map(v=>({...v})):[];
  clone.trips=Array.isArray(source.trips)?source.trips.map(t=>({...t,attachment:stripAttachmentForState(t.attachment)})):[];
  clone.expenses=Array.isArray(source.expenses)?source.expenses.map(e=>({...e,attachment:stripAttachmentForState(e.attachment)})):[];
  clone.attachmentStorage={type:'indexeddb',db:ATTACHMENT_DB_NAME,updatedAt:new Date().toISOString()};
  return clone;
}
async function attachmentWithData(a){
  const safe=safeAttachment(a);
  if(!safe)return null;
  if(safe.data)return safe;
  const rec=await idbGet(safe.id);
  if(rec?.data)return {...safe,data:rec.data,size:rec.size||safe.size};
  return safe;
}
async function dataUrlFromAttachment(a){
  const full=await attachmentWithData(a);
  return full?.data||'';
}
async function buildFullBackupPayload(){
  const payload=lightweightState(state);
  payload.settings={...state.settings};
  payload.profileBackup={name:state.settings.name||'',country:state.settings.country||'',home1:state.settings.home1||'',homeLabel:state.settings.homeLabel||'',work:state.settings.work||'',workLabel:state.settings.workLabel||'',locations:Array.isArray(state.settings.locations)?state.settings.locations:[],addresses:state.addresses&&typeof state.addresses==='object'?state.addresses:{}};
  payload.exportedAt=new Date().toISOString();
  payload.exportVersion=VERSION;
  for(const row of [...payload.trips,...payload.expenses]){
    if(row.attachment){
      const full=await attachmentWithData(row.attachment);
      if(full?.data)row.attachment={...row.attachment,data:full.data,size:attachmentBytes(full)};
    }
  }
  return payload;
}
async function createInternalSnapshot(reason='auto'){
  const payload=await buildFullBackupPayload();
  const serialized=JSON.stringify(payload);
  const id=`snapshot_${new Date().toISOString()}_${reason}`.replace(/[^a-zA-Z0-9_.-]/g,'-');
  await idbPut({id,name:id,type:'application/json',date:new Date().toISOString(),size:serialized.length,payload,backup:true});
  const backups=(await idbAll('snapshot_')).sort((a,b)=>String(b.date).localeCompare(String(a.date)));
  for(const old of backups.slice(MAX_LOCAL_BACKUPS))await idbDelete(old.id);
}
async function refreshStorageInfo(){
  const rows=await idbAll();
  const attachmentBytesTotal=rows.filter(x=>!x.backup).reduce((sum,x)=>sum+Number(x.size||attachmentBytes(x)||0),0);
  const backupCount=rows.filter(x=>x.backup).length;
  storageInfoCache={attachmentBytes:attachmentBytesTotal,backupCount};
  if(document.getElementById('storageStatus'))renderSettings();
}
async function migrateEmbeddedAttachmentsToIndexedDb({silent=false}={}){
  const changes=[];
  [...state.trips,...state.expenses].forEach(row=>{if(row.attachment?.data){const safe=safeAttachment(row.attachment);if(safe)changes.push({row,safe})}});
  if(!changes.length)return true;
  try{
    for(const item of changes){
      const meta=await queueAttachmentPersist(item.safe);
      item.row.attachment=meta;
      item.row.updatedAt=item.row.updatedAt||new Date().toISOString();
    }
    localStorage.setItem(KEY,JSON.stringify(lightweightState(state)));
    createInternalSnapshot('post-attachment-migration');
    refreshStorageInfo();
    if(!silent)toast(t('Receipt storage optimised. Existing trip and cost data was kept.'));
    return true;
  }catch(e){
    console.warn('Attachment migration failed; inline data was kept.',e);
    if(!silent)toast(t('Attachment migration failed. Existing data was kept.'));
    return false;
  }
}
...
function save(options={}){
  if(!options.preserveTimestamp)touchState();
  const storageState=lightweightState(state);
  const payload=JSON.stringify(storageState);
  try{
    localStorage.setItem(KEY,payload);
    const check=safeParse(localStorage.getItem(KEY));
    if(!validData(check)||check.trips.length!==state.trips.length||check.expenses.length!==state.expenses.length)throw new Error('Verification failed');
    if(!options.skipRender)render();
    if(!options.skipSync&&!suppressAutoSync)scheduleAutoSync();
    return true
  }catch(e){
    console.warn('MoBud save failed',e);
    if(Date.now()-lastSaveFailureToast>12000){
      lastSaveFailureToast=Date.now();
      toast(t('Opslaan mislukt. Je bestaande data bleef bewaard. Verwijder oude bonbijlagen of maak eerst een back-up.'))
    }
    return false
  }
}
...
function showVersionSafetyGate(){
  ...
  modal.querySelector('[data-version-backup]').onclick=async()=>{await downloadBackup('before-v0.000.016');versionGateBackupTaken=true;confirmBtn.disabled=false;toast(t('Safety backup downloaded.'))};
...
function logHtml(x){const v=vehicleById(x.vehicleId),type=v?.type||(x.kind==='expense'?'other':'other'),hasAttachment=!!x.attachment;let badge='';if(x.kind==='trip'&&hasAttachment)badge=`<span class="log-badge log-badge-combo">${escapeHtml(t('Trip'))} + ${escapeHtml(t('Receipts'))}</span>`;else if(x.kind==='expense'&&hasAttachment)badge=`<span class="log-badge log-badge-receipt">${escapeHtml(t('Receipts'))}</span>`;else if(x.kind==='expense')badge=`<span class="log-badge log-badge-cost">${escapeHtml(t('Cost'))}</span>`;const title=x.kind==='trip'?friendlyWhen(x.date)+(x.createdAt?' '+new Date(x.createdAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):''):cleanText(x.category,200);const subtitle=x.kind==='trip'?(v?.name||t('No specific vehicle')):(v?.name||x.supplier||t('General cost'));const value=x.kind==='trip'?unitDistance(x.km):money(x.amount);const kind=x.kind==='expense'?'expense':'trip',visual=x.kind==='trip'&&hasAttachment?'combo':(x.kind==='expense'?'receipt':'trip');return `<div class="log-row log-${visual}"><div class="log-icon">${iconType(type)}</div><div class="log-main"><strong>${escapeHtml(title)}</strong><small>${escapeHtml(subtitle)}${badge}</small></div><div class="log-side"><strong>${escapeHtml(value)}</strong><div class="log-actions"><button class="ghost" data-edit-${kind}="${safeAttr(x.id)}">Edit</button><button class="danger" data-delete-${kind}="${safeAttr(x.id)}">Delete</button></div></div></div>`}
...
async function compressImageFile(file,maxSide=1800,quality=.76){
  if(!file?.type?.startsWith('image/'))return file;
  const img=new Image();
  const url=URL.createObjectURL(file);
  try{
    img.src=url;await new Promise((resolve,reject)=>{img.onload=resolve;img.onerror=reject});
...
async function fileData(file){
  if(!file)return null;
  if(file.size>25_000_000)throw new Error('File too large');
  let prepared=file;
  if(file.type.startsWith('image/'))prepared=await compressImageFile(file);
  if(!ALLOWED_ATTACHMENT_TYPES.has(prepared.type))throw new Error('Unsupported file type');
  return new Promise((res,rej)=>{const r=new FileReader();r.onload=async()=>{try{const a=safeAttachment({id:uid('att'),name:prepared.name,type:prepared.type,data:r.result,date:new Date().toISOString(),size:prepared.size,storage:'indexeddb'});if(!a)throw new Error('Invalid attachment');await queueAttachmentPersist(a);res(stripAttachmentForState(a))}catch(error){rej(error)}};r.onerror=rej;r.readAsDataURL(prepared)})
}
...
function renderGarage(){
  ...
  const card=v=>{const s=vehicleMonthStats(v),title=vehicleDisplayName(v),official=vehicleOfficialName(v),partnerButtons=[v.maintenanceProvider?`<a class="secondary small" href="${escapeHtml(safeUrl(v.maintenanceUrl,{allowHttp:true})||'mailto:?subject='+encodeURIComponent('Vraag over voertuig '+official))}" target="_blank" rel="noopener noreferrer">${t('Request maintenance/repair')}</a>`:'',v.leasePartner?`<a class="secondary small" href="${escapeHtml(safeUrl(v.leaseUrl,{allowHttp:true})||'mailto:?subject='+encodeURIComponent('Vraag over voertuig '+official))}" target="_blank" rel="noopener noreferrer">${t('Contact leasepartner')}</a>`:'',v.insurance?`<a class="secondary small" href="${escapeHtml(safeUrl(v.insuranceUrl,{allowHttp:true})||'mailto:?subject='+encodeURIComponent('Vraag over voertuig '+official))}" target="_blank" rel="noopener noreferrer">${t('Insurance')}</a>`:''].filter(Boolean).join('');const parking=v.parking&&Number.isFinite(v.parking.lat)&&Number.isFinite(v.parking.lon)?`<div class="parking-strip"><strong>${t('Parking location saved')}</strong><small>${new Date(v.parking.createdAt||Date.now()).toLocaleString()}</small><div class="mini-actions"><button class="secondary small" data-route-parking="${safeAttr(v.id)}">${t('Route')}</button><button class="ghost small" data-clear-parking="${safeAttr(v.id)}">${t('I left')}</button></div></div>`:'';return `<div class="card garage-card refined-garage-card ${v.active===false?'inactive-vehicle-card':''}"><div class="section-head garage-card-head"><div class="vehicle-title"><span class="vehicle-icon">${iconType(v.type)}</span><div><h3>${escapeHtml(title)}</h3><strong>${escapeHtml(official)}</strong>${v.isDefault&&v.active!==false?`<span class="default-badge vehicle-default-block">${t('DEFAULT')}</span>`:''}</div></div><div class="log-actions"><button class="ghost" data-edit-vehicle="${safeAttr(v.id)}">${t('Edit')}</button><button class="ghost" data-archive-vehicle="${safeAttr(v.id)}">${v.active===false?t('Restore'):t('Make inactive')}</button></div></div><div class="odometer-highlight"><small>${t('Current odometer')}</small><strong>${unitDistance(calculatedOdometer(v))}</strong></div><div class="vehicle-month-cards"><button data-vehicle-logs="${safeAttr(v.id)}" data-log-kind="trip"><small>${t('Last month')}</small><strong>${s.trips.length}</strong><span>${t('trips')}</span></button><button data-vehicle-logs="${safeAttr(v.id)}" data-log-kind="receipt"><small>${t('Last month')}</small><strong>${s.receipts}</strong><span>${t('receipts')}</span></button><button data-vehicle-logs="${safeAttr(v.id)}" data-log-kind="cost"><small>${t('Last month')}</small><strong>${money(s.cost)}</strong><span>${t('total cost')}</span></button></div>${maintenanceTimeline(v)}${parking}<div class="garage-context-actions"><button class="secondary small" data-new-trip-vehicle="${safeAttr(v.id)}">${t('Add trip')}</button><button class="secondary small" data-new-expense-vehicle="${safeAttr(v.id)}">${t('Add receipt')}</button><button class="secondary small" data-save-parking="${safeAttr(v.id)}">${t('Save parking')}</button></div>${partnerButtons?`<div class="partner-actions">${partnerButtons}</div>`:''}${v.active===false?`<div class="danger-zone"><button class="danger small" data-delete-vehicle="${safeAttr(v.id)}">${t('Permanently delete')}</button></div>`:''}</div>`};
...
function saveParkingForVehicle(id){const v=vehicleById(id);if(!v)return;if(!navigator.geolocation){toast(t('Location is not available on this device.'));return}toast(t('Requesting location…'));navigator.geolocation.getCurrentPosition(pos=>{v.parking={lat:pos.coords.latitude,lon:pos.coords.longitude,accuracy:pos.coords.accuracy,createdAt:new Date().toISOString()};v.updatedAt=new Date().toISOString();if(save())toast(t('Parking location saved.'));renderGarage()},()=>toast(t('Location permission was not granted.')),{enableHighAccuracy:true,timeout:12000,maximumAge:60000})}
...
function rowsForReport(){const {from,to}=reportDateBounds(),matchesVehicle=x=>reportVehicleId==='all'||x.vehicleId===reportVehicleId;return {trips:state.trips.filter(x=>(x.date||'')>=from&&(x.date||'')<=to&&matchesVehicle(x)),expenses:state.expenses.filter(x=>(x.date||'')>=from&&(x.date||'')<=to&&matchesVehicle(x))}}
function vehicleBreakdownRows(values,total,formatter){return values.length?values.map(x=>`<div class="analysis-list-row"><span class="vehicle-distance-icon">${iconType(x.v.type)}</span><span>${escapeHtml(vehicleOfficialName(x.v))}</span><strong>${formatter(x.value)} <small>${total?`${(x.value/total*100).toFixed(1)}%`:'0.0%'}</small></strong></div>`).join(''):'<p class="muted">No data in this period.</p>'}
...
function showReportOverview(kind){
  const rows=rowsForReport(),bounds=reportDateBounds();let list=[];
  if(kind==='trips')list=rows.trips.map(x=>({...x,kind:'trip'}));
  else if(kind==='costs')list=rows.expenses.map(x=>({...x,kind:'expense'}));
  else if(kind==='receipts')list=rows.expenses.filter(x=>x.attachment).map(x=>({...x,kind:'expense'}));
  list.sort((a,b)=>registrationSortValue(b).localeCompare(registrationSortValue(a)));
  setScreen('calendar');
  selectedDayTitle.textContent=`${t(kind==='trips'?'Total trips':kind==='costs'?'Total costs':'Receipts')} · ${escapeHtml(bounds.label)}`;
...
function bindReportControls(){
  ...
  const vehicle=document.getElementById('reportVehicleFilter');if(vehicle)vehicle.onchange=()=>{reportVehicleId=vehicle.value;renderStats()};
  document.querySelectorAll('[data-analysis-toggle]').forEach(b=>b.onclick=()=>showReportOverview(b.dataset.analysisToggle));
}
...
function renderStats(){
  ...
  statsContent.innerHTML=`
    ...
    <div class="stats-kpi-grid">
      ...
      <div class="card analysis-card"><button class="stat-summary-row analysis-toggle" data-analysis-toggle="costs"><span>${t('Total costs')}</span><strong>${money(totalCost)}<i>${openReportPanels.has('costs')?'−':'+'}</i></strong></button><div class="analysis-details ${openReportPanels.has('costs')?'':'hidden'}">${analysisBar(byCost,totalCost,palette)}<div class="analysis-list">${vehicleBreakdownRows(byCost,totalCost,money)}</div></div></div>
      <div class="card analysis-card"><button class="stat-summary-row analysis-toggle" data-analysis-toggle="receipts"><span>${t('Receipts')}</span><strong>${receipts}<i>${openReportPanels.has('receipts')?'−':'+'}</i></strong></button><div class="analysis-details ${openReportPanels.has('receipts')?'':'hidden'}">${analysisBar(byReceipts,receipts,palette)}<div class="analysis-list">${vehicleBreakdownRows(byReceipts,receipts,n=>`${n} ${n===1?t('receipt'):t('receipts')}`)}</div></div></div>
    </div>
...
async function createReportPdfBlob(){
  ...
  const receiptTiles=[];
  for(const expense of attachments){const a=await attachmentWithData(expense.attachment);if(!a)continue;try{if(a.type==='application/pdf'&&a.data){const embedded=await pdf.embedPdf(dataUrlBytes(a.data));embedded.forEach((drawable,pageIndex)=>receiptTiles.push({expense,a,drawable,pageIndex,kind:'pdf'}))}else{receiptTiles.push({expense,a,drawable:await imageAttachmentForPdf(pdf,a),pageIndex:0,kind:'image'})}}catch(err){receiptTiles.push({expense,a,error:true,pageIndex:0,kind:'error'})}}
...
${t('Let op: dit verwijdert de bijlagen permanent. Ritten en kostgegevens blijven behouden.')}`))return;
  const now=new Date().toISOString();rows.forEach(e=>{if(e.attachment?.id)idbDelete(e.attachment.id);e.attachment=null;e.attachmentRemovedAt=now;e.updatedAt=now});save();refreshReceiptCleanupPreview();refreshStorageInfo();toast(t('Bijhorende bonnetjes verwijderd.'));
}
...
function renderSettings(){
  ...
  storageStatus.textContent=`${state.settings.driveConnected?t('Google Drive connected'):t('Data is stored locally on this device.')} · ${t('Attachments')}: ${(storageInfoCache.attachmentBytes/1024/1024).toFixed(1)} MB · ${t('Internal backups')}: ${storageInfoCache.backupCount}`;
...
async function downloadDriveJson(fileId){const r=await driveFetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?alt=media`);if(!r.ok)throw new Error(`Drive download failed (${r.status})`);const text=await r.text();if(text.length>100_000_000)throw new Error('Drive backup too large');const data=JSON.parse(text);if(!validData(data))throw new Error('Invalid Drive backup');return data}
...
function buildBackupPayload(){return lightweightState(state)}
async function downloadBackup(reason='manual'){
  const payload=await buildFullBackupPayload();
  const b=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),a=document.createElement('a');
  a.href=URL.createObjectURL(b);a.download=`mobud-backup-${reason}-${todayISO()}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  return payload
}
exportJson.onclick=()=>downloadBackup('manual');importJson.onchange=e=>{const f=e.target.files[0];if(!f)return;if(f.size>100_000_000){toast('Backup is too large');return}const r=new FileReader();r.onload=async()=>{try{const raw=JSON.parse(r.result);if(!validData(raw))throw new Error('Invalid backup');const imported=normalized(raw),preview=`Current local data:
...
window.addEventListener('mobud:languagechange',()=>{state.settings.language=window.MOBUD_I18N.language;save({skipSync:true})});
