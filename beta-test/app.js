...
function fillVehicleSelects(){const opts=[`<option value="">${t('No specific vehicle')}</option>`,...state.vehicles.filter(v=>v.active!==false).map(v=>`<option value="${safeAttr(v.id)}">${escapeHtml(v.name)}${v.isDefault?' · '+t('default'):''}</option>`)].join('');tripVehicle.innerHTML=opts;expenseVehicle.innerHTML=opts}
...
function maintenanceTimeline(v){
  const logged=vehicleDistance(v),base=Number(v.currentOdometer||v.initialOdometer||0);
  const odometerLogs=state.expenses.filter(e=>e.vehicleId===v.id&&Number(e.odometer||0)>0);
  const current=Math.max(base+logged,...odometerLogs.map(e=>Number(e.odometer||0)),base,0);
...
function updateLocationLabel(id,value){
  ...
  {const now=new Date().toISOString();state.syncMeta.settingsUpdatedAt=now;state.syncMeta.addressesUpdatedAt=now}save({skipRender:true});renderDirectionLabels();
}
function updateLocationAddress(id,value){
  ...
  {const now=new Date().toISOString();state.syncMeta.settingsUpdatedAt=now;state.syncMeta.addressesUpdatedAt=now}save({skipRender:true});
}
function addLocation(){
  ...
  {const now=new Date().toISOString();state.syncMeta.settingsUpdatedAt=now;state.syncMeta.addressesUpdatedAt=now}save();renderAddressSettings();fillLocations();
}
